  ...
  class UserGroupsBase extends Entity{ 
    ...
    function insert(){
      if(!$this->canInsert())
        showMessageDlg("Data UserGroups tidak dapat di-insert",true);
	  	...
	  	$this->setField("USER_GROUP_ID", $this->getNextId("USER_GROUP_ID","CAPIL.USER_GROUP"));		
		$str = "
				INSERT INTO CAPIL.USER_GROUP (
				   USER_GROUP_ID, DATABASE, LAPORAN, 
				   ARSIP, MASTER_DATA, AKTA, 
				   KARTU_KELUARGA, KARTU_TANDA_PENDUDUK, NAMA) 
 			  	VALUES (
				  ".$this->getField("USER_GROUP_ID").",
				  ".$this->getField("DATABASE").",
  				  ".$this->getField("LAPORAN").",
				  ".$this->getField("ARSIP").",
				  ".$this->getField("MASTER_DATA").",
				  ".$this->getField("AKTA").",
				  ".$this->getField("KARTU_KELUARGA").",
				  ".$this->getField("KARTU_TANDA_PENDUDUK").",
				  '".$this->getField("NAMA")."'
				)"; 
		$this->query = $str;
    ...
    **/
    function update(){
      if(!$this->canUpdate())
		...
		$str = "
				UPDATE CAPIL.USER_GROUP
				SET    
					  DATABASE				= ".$this->getField("DATABASE").",
					  LAPORAN				= ".$this->getField("LAPORAN").",
					  ARSIP					= ".$this->getField("ARSIP").",
					  MASTER_DATA			= ".$this->getField("MASTER_DATA").",
					  AKTA					= ".$this->getField("AKTA").",
					  KARTU_KELUARGA		= ".$this->getField("KARTU_KELUARGA").",
					  KARTU_TANDA_PENDUDUK	= ".$this->getField("KARTU_TANDA_PENDUDUK").",
    ...
    **/
    function delete(){
      if(!$this->canDelete())
        ...
        $str = "DELETE FROM CAPIL.USER_GROUP 
                WHERE 
    ...
    **/
    function selectById($USER_GROUP_ID){
      $str = "SELECT * FROM CAPIL.USER_GROUP
              WHERE 
    ...
    **/ 
    function getCountByParams($paramsArray=array(), $varStatement=""){
      $str = "SELECT COUNT(USER_GROUP_ID) AS ROWCOUNT FROM CAPIL.USER_GROUP WHERE USER_GROUP_ID IS NOT NULL ".$varStatement; 
      while(list($key,$val)=each($paramsArray)){
        $str .= " AND $key LIKE '%$val%' ";
      }
      $this->select($str); 

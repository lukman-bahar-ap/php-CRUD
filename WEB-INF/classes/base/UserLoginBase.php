  ...
  ***/
  ...
  class UserLoginBase extends Entity{ 
    ...
    **/
	...
	function updatePasswordMirorId()
	{
		$this->setField("USER_PASS", md5($this->getField("USER_PASS")));
		$str = "UPDATE user_login SET
				  USER_PASS = '".$this->getField("USER_PASS")."'
				WHERE MIROR_ID = ".$this->getField("MIROR_ID")." AND USER_GROUP_ID	= '".$this->getField("USER_GROUP_ID")."'
				"; 
		$this->query = $str;
		return $this->execQuery($str);
    }
    ...
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "
				SELECT USER_LOGIN_ID, A.USER_GROUP_ID, 
                    ...
                    A.NAMA, ALAMAT, EMAIL, 
				    TELEPON, A.USER_NAMA, USER_PASS, A.UNIT_UPJ
				FROM user_login A 
	...
	function selectByParamsNotKontrak($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "
				SELECT USER_LOGIN_ID, A.USER_GROUP_ID, 
				    ...
				    TELEPON, A.USER_NAMA, USER_PASS, A.UNIT_UPJ
				FROM user_login A 
				LEFT JOIN user_group C ON A.USER_GROUP_ID = C.USER_GROUP_ID AND C.USER_GROUP_ID <> '5'
				WHERE A.USER_GROUP_ID <> '5' AND C.USER_GROUP_ID <> '5' AND USER_LOGIN_ID IS NOT NULL
			"; 
	...
	function selectByParamsLike($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "
				SELECT USER_LOGIN_ID, A.USER_GROUP_ID, 
                    ...
                    A.NAMA, ALAMAT, EMAIL, 
				    TELEPON, USER_NAMA, USER_PASS, A.UNIT_UPJ
				FROM user_login A 
    ...
    **/ 
	...
	function getCountByParamsNotKontrak($paramsArray=array(),$stat="")
	{
		$str = "SELECT COUNT(USER_LOGIN_ID) AS ROWCOUNT FROM user_login A WHERE USER_GROUP_ID <> '5' AND USER_LOGIN_ID IS NOT NULL ".$stat; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = '$val' ";
		}

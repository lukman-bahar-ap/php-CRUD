        ...
        */
        ...
        return this.each(function () {
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;Tampil Berdasarkan Kedudukan <select id='tahun'><option value=''>Semua</option></select>"); 
				oSifatRowSelect = $("#tahun");
				$.getJSON("../json/mutasi_json.php",
				function(data){
					for(i=0;i<data.KEDUDUKAN_ID.length; i++)
					{
					$("<option value='" + data.KEDUDUKAN_ID[i] + "'>" + data.NAMA[i] + "</option>").appendTo("#tahun"); 
					} 
				});
				oSifatRowSelect.change(function () { 
					//alert($("#tahun").val());	
					/* Example call to load a new file */
					fnReloadAjax('../json/daftar_pegawai_mutasi_json.php?reqKedudukanId=' + $("#tahun").val() + '&reqId=' + properties.pSatkerId);			

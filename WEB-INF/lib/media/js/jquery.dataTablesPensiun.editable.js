...
//			alert(mySplitResult[6]+'--');
        ...
        function _fnOnRowBaca(event) {
			if($("#batas").val() == "")
				alert('Isi terlebih dahulu batasan umur.')
			else
				fnReloadAjax('../json/pensiun_json.php?reqSatkerId=' + properties.pSatkerId + '&reqBulan=' + $("#bulan").val() + '&reqTahun=' + $("#tahun").val() + '&reqBatas=' + $("#batas").val() +  '&reqTipePegawai=' + $("#tipepegawai").val() + '&reqType=A');
        }

		function _fnOnRowSetPensiun(event) {
			var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
			opWidth = 1050;
			opHeight = 550;
		...
		function _fnOnRowPensiunFoto(event) {
			var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
			...
			opUrl    = 'cetak_pensiun_foto.php?reqId='+properties.pSatkerId+'&reqPegawaiId='+id+'&reqBulan='+$("#bulan").val()+'&reqTahun='+$("#tahun").val()+'&reqBatas='+$("#batas").val();
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
			newWindow.focus();
        ...
        */
        function fnOnDeleting(tr, id, fnDeleteRow) {
			//alert(id);
            return confirm("Apakah anda yakin ingin mengubah status pegawai terpilih sebagai pensiun?"); ;
        }
        ...
        */
        ...
        return this.each(function () {
			...
			oPilihArsipRowButton = $('#btnSetPensiun');
            if (oPilihArsipRowButton.length != 0)
            {
                oPilihArsipRowButton.click(_fnOnRowSetPensiun);
            }
            else {
                oPilihArsipRowButton = null;
            }
			
			oPensiunFotoRowButton = $('#btnPensiunFoto');
            if (oPensiunFotoRowButton.length != 0)
            {
                oPensiunFotoRowButton.click(_fnOnRowPensiunFoto);
            }
            else {
                oPensiunFotoRowButton = null;
            }
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;Periode <select id='bulan'><option value='01'>Januari</option><option value='02'>Februari</option><option value='03'>Maret</option><option value='04'>April</option><option value='05'>Mei</option><option value='06'>Juni</option><option value='07'>Juli</option><option value='08'>Agustus</option><option value='09'>September</option><option value='10'>Oktober</option><option value='11'>November</option><option value='12'>Desember</option></select> <select id='tahun'></select> Tipe Pegawai <select id='tipepegawai'><option value='1'>Struktural</option><option value='2'>Fungsional</option></select> Batas Umur <input type='text' id='batas' style='width:30px'>");	
				oBulanRowSelect = $("#bulan");
				var bulan_sekarang = "";
				if(currentTime.getMonth() + 1 < 10)
					bulan_sekarang = "0" + currentTime.getMonth();
				else
					bulan_sekarang = currentTime.getMonth() + 1;
				$("#bulan").val(bulan_sekarang);
				
				oTahunRowSelect = $("#tahun");
				for(i=currentTime.getFullYear();i<=currentTime.getFullYear() + 40; i++)
				{

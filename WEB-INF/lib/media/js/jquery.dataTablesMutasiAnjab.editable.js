...
//			alert(mySplitResult[6]+'--');
        ...
        function _fnOnRowBaca(event) {
			...
			opWidth = 1050;
			opHeight = 350;
			opUrl    = 'anjab_kandidat.php?reqId='+id;
		...
		function _fnOnSetKandidat(event) {
            var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
			opWidth = 1050;
			opHeight = 550;
			opUrl    = 'pop_set_anjab.php?reqId='+id;
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
			newWindow.focus();			
		...
		function _fnOnRowPindahArsip(event) {
			...
			if(sData){
				opWidth = 800;
				opHeight = 400;
				opUrl    = 'mutasi_pegawai_add.php?reqPegawaiId='+sData+'&reqId='+properties.pSatkerId;
				var left = (screen.width/2)-(opWidth/2);
				var top = (screen.height/2)-(opHeight/2);
				newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
				newWindow.focus();			
        ...
        */
        ...
        return this.each(function () {
			...
			oSetKandidatButton = $('#btnSetKandidat');
			/*$('#btnSetKandidat').click(function(){
				alert('Masuk Jquery');
				});*/
            if (oSetKandidatButton.length != 0)
			{
                oSetKandidatButton.click(_fnOnSetKandidat);
				//alert('Helllloooo');
			}
            else {
                oSetKandidatButton = null;
				//alert('Kosong');
            }
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Data pegawai yang akan di mutasi ");
            }

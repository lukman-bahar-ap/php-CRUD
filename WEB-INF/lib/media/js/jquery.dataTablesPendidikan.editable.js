        ...
        */
        ...
        return this.each(function () {
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;Tampil Berdasarkan Jenjang <select id='tahun'><option value=''>Semua</option></select>"); 
				oSifatRowSelect = $("#tahun");
				$.getJSON("../json/pendidikan_json.php",
				function(data){
					for(i=0;i<data.PENDIDIKAN_ID.length; i++)
					{
					$("<option value='" + data.PENDIDIKAN_ID[i] + "'>" + data.NAMA[i] + "</option>").appendTo("#tahun"); 
					} 
				});
				oSifatRowSelect.change(function () { 
					//alert($("#tahun").val());	
					/* Example call to load a new file */
					fnReloadAjax('../json/daftar_pegawai_jenjang_pendidikan_json.php?reqPendidikanId=' + $("#tahun").val() + '&reqId=' + properties.pSatkerId);			

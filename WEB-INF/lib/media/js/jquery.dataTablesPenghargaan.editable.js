        ...
        */
        ...
        return this.each(function () {
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;Tampil Berdasarkan Penghargaan <select id='tahun'><option value=''>Semua</option></select>"); 
				oSifatRowSelect = $("#tahun");
				$.getJSON("../json/penghargaan_json.php",
				function(data){
					for(i=0;i<data.NAMA.length; i++)
					{
					$("<option value='" + data.NAMA[i] + "'>" + data.NAMA[i] + "</option>").appendTo("#tahun"); 
					} 
				});
				oSifatRowSelect.change(function () { 
					//alert($("#tahun").val());	
					/* Example call to load a new file */
					fnReloadAjax('../json/daftar_pegawai_penghargaan_json.php?reqPenghargaanId=' + $("#tahun").val() + '&reqId=' + properties.pSatkerId);			

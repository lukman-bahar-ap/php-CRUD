    ...
    $.fn.makeEditable = function (options) {
		...
		*/
        ...
        function _fnOnRowBaca(event) {
			fnReloadAjax('../json/daftar_urutan_pegawai_json.php?reqTipePegawaiId=' + $("#filter").val() + '&reqId=' + properties.pSatkerId + '&reqBulan=' + $("#bulan").val() + '&reqTahun=' + $("#tahun").val());
        }
		...
		function _fnOnRowProses(event) {
			var sData = $('input', oTable.fnGetNodes()).serialize();
			
			//alert(sData+properties.pPegawaiId+'-'+$("#tahun").val());
			
			//sData = sData.replace(/check=/gi, "");		
			//sData = sData.replace(/&/gi, ",");
			
			if(sData) 	window.self.location.href = 'monitoring_perubahan_data_simpan.php?reqId='+properties.pPegawaiId+'&reqGroup='+$("#tahun").val()+'&reqPilihId='+ sData;
			else 		alert('Pilih salah satu data, untuk di proses');
        ...
        */
        ...
        return this.each(function () {
			...
			oProsesRowButton = $('#btnProsesRow');
            if (oProsesRowButton.length != 0)
            {
                oProsesRowButton.click(_fnOnRowProses);
            }
            else {
                oProsesRowButton = null;
            }
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;Tampil Berdasarkan <select id='tahun'><option value=''>Semua</option></select>"); 
				oSifatRowSelect = $("#tahun");
				$.getJSON("../json/monitoring_perubahan_data_select_json.php"+ "?reqPegawaiId=" + properties.pPegawaiId,
				function(data){
					for(i=0;i<data.AGAMA_ID.length; i++)
					{
					$("<option value='" + data.AGAMA_ID[i] + "'>" + data.NAMA[i] + "</option>").appendTo("#tahun"); 
					} 
				});
				oSifatRowSelect.change(function () { 
					//alert($("#tahun").val());	
					/* Example call to load a new file */
					fnReloadAjax('../json/monitoring_perubahan_data_pop_json.php?reqAgamaId=' + $("#tahun").val() + '&reqId=' + properties.pPegawaiId);		

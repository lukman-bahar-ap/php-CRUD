...
//			alert(mySplitResult[6]+'--');
        ...
        function _fnOnRowBaca(event) {			
				fnReloadAjax('../json/'+ properties.pLocationId +'?reqMode=proses&reqId=' + properties.pSatkerId + '&reqBulan=' + $("#bulan").val() + '&reqTahun=' + $("#tahun").val() + '&reqType=' + $("#jeniskp").val());  
        ...
        function _fnOnRowBacaKeluar(event) {
			...
			opWidth = 1050;
			opHeight = 550;
			opUrl    = 'pop_riwayat_pangkat.php?reqId='+id;
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
			newWindow.focus();			
		...
		function _fnOnRowKgbLengkap(event) {
		var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
		//alert(id);
			opWidth = 10;
			opHeight = 5;
			opUrl    = 'kenaikan_gaji_berkala_cetak_sk.php?reqId='+id;
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
			newWindow.focus();			
		...
		function _fnOnRowImpasing(event) {
		var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
		//alert(id);
			opWidth = 10;
			opHeight = 5;
			opUrl    = 'kenaikan_gaji_berkala_cetak_impasing.php?reqId='+id;
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
			newWindow.focus();			
        ...
        */
        ...
        return this.each(function () {
			...
			//Set the click handler on the "Delete selected row" button 
            oBacaKeluarRowButton = $('#btnUpdateRiwayatGaji');
			...
			oImpasingRowButton = $('#btnImpasingRow');
            if (oImpasingRowButton.length != 0)
			{
                oImpasingRowButton.click(_fnOnRowImpasing);
			}
            else {
                oImpasingRowButton = null;
            }
			
			oKgbLengkapRowButton = $('#btnKgbLengkapRow');
            if (oKgbLengkapRowButton.length != 0)
			{
                oKgbLengkapRowButton.click(_fnOnRowKgbLengkap);
			}
            else {
                oKgbLengkapRowButton = null;
            }
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;Cetak gol <select id='idgolongan'><option value=''>Semua</option></select>"); 
				oGolonganRowSelect = $("#idgolongan");
				$.getJSON("../json/sifat-json.php",
				function(data){
					for(i=0;i<data.SIFAT_ID.length; i++)
					{
					$("<option value='" + data.SIFAT_ID[i] + "'>" + data.SIFAT_NAMA[i] + "</option>").appendTo("#idgolongan"); 
					} 
				});*/
				...
				//oAddDeleteToolbar.append("&nbsp; Jenis KP <select id='jeniskp'><option value='REGULER'>Reguler</option><option value='PILIHAN_STRUKTURAL'>Pilihan (Struktural)</option><option value='PILIHAN_FUNGSIONAL'>Pilihan (Fungsional)</option><option value='ANUMERTA'>Anumerta</option><option value='PENGABDIAN'>Pengabdian</option></select> &nbsp;Periode <select id='bulan'><option value='01'>Januari</option><option value='02'>Februari</option><option value='03'>Maret</option><option value='04'>April</option><option value='05'>Mei</option><option value='06'>Juni</option><option value='07'>Juli</option><option value='08'>Agustus</option><option value='09'>September</option><option value='10'>Oktober</option><option value='11'>November</option><option value='12'>Desember</option></select> <select id='tahun'></select>");	
				oAddDeleteToolbar.append("&nbsp; Jenis KP <select id='jeniskp'><option value='REGULER'>Reguler</option><option value='PILIHAN_STRUKTURAL'>Pilihan (Struktural)</option><option value='PILIHAN_FUNGSIONAL'>Pilihan (Fungsional)</option><option value='ANUMERTA'>Anumerta</option><option value='PENGABDIAN'>Pengabdian</option></select> &nbsp;Periode <select id='bulan'><option value='04'>April</option><option value='10'>Oktober</option></select> <select id='tahun'></select>");	
				...
				for(i=currentTime.getFullYear();i<=currentTime.getFullYear() + 2; i++)

				{
					$("<option value='" + i + "'>" + i + "</option>").appendTo("#tahun");			
				}
				$("#tahun").val(currentTime.getFullYear());
				oJenisKPRowSelect = $("#jeniskp");
				
				if(properties.pLocationId == 'kenaikan_pangkat_proses_riwayat_json.php')
				{
					oAddDeleteToolbar.append("&nbsp;Nomor <input type='text' name='reqRowNomor' id='reqRowNomor' style='width:60px' \>");
					oNomorRowSelect = $("#reqRowNomor");
					
					oNomorRowSelect.change(function () {
						fnReloadAjax('../json/'+ properties.pLocationId +'?reqId=' + properties.pSatkerId + '&reqRowNomor=' + $("#reqRowNomor").val() + '&reqRowStatus=' + $("#reqRowStatus").val() + '&reqBulan=' + $("#bulan").val() + '&reqTahun=' + $("#tahun").val() + '&reqType=' + $("#jeniskp").val()); 
					});
					
					oAddDeleteToolbar.append("&nbsp;Status <select id='reqRowStatus'><option value=''>Semua Data</option><option value='1'>Setuju</option><option value='0'>Tidak Setuju</option></select>");
					oStatusRowSelect = $("#reqRowStatus");
					
					oStatusRowSelect.change(function () { 
						fnReloadAjax('../json/'+ properties.pLocationId +'?reqId=' + properties.pSatkerId + '&reqRowNomor=' + $("#reqRowNomor").val() + '&reqRowStatus=' + $("#reqRowStatus").val() + '&reqBulan=' + $("#bulan").val() + '&reqTahun=' + $("#tahun").val() + '&reqType=' + $("#jeniskp").val()); 
					});
				
				}
				
				oJenisKPRowSelect.change(function () { 
					//alert($("#pilihan").val());	
				...
				oBulanRowSelect.change(function () { 
					...
					fnReloadAjax('../json/'+ properties.pLocationId +'?reqId=' + properties.pSatkerId + '&reqRowNomor=' + $("#reqRowNomor").val() + '&reqRowStatus=' + $("#reqRowStatus").val() + '&reqBulan=' + $("#bulan").val() + '&reqTahun=' + $("#tahun").val() + '&reqType=' + $("#jeniskp").val()); 
					//fnReloadAjax('../json/kenaikan_gaji_berkala_json.php?reqId=' + properties.pSatkerId + '&reqBulan=' + $("#bulan").val() + '&reqTahun=' + $("#tahun").val());
				});

    ...
    $.fn.makeEditable = function (options) {
		...
		*/
        ...
        function _fnOnRowBaca(event) {
			...
			opWidth = 1050;
			opHeight = 550;
			opUrl    = 'arsip_download.php?reqId='+id;
		...
		function _fnOnRowEditKeluarNew(event) {
			if(confirm("Tolak permohonan peminjaman?") == true)
			{			
				...
				opWidth = 300;
				opHeight = 300;
				opUrl    = 'proses_pinjam.php?reqMode=tolak&reqId='+sData;
				var left = (screen.width/2)-(opWidth/2);
				var top = (screen.height/2)-(opHeight/2);
				newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
				newWindow.focus();				
		...
		function _fnOnRowEditNotaDinas(event) {
			...
			if(confirm("Setujui permohonan peminjaman?") == true)
			{
				var sData = $('input', oTable.fnGetNodes()).serialize();
				sData = sData.replace(/check=/gi, "");		
				sData = sData.replace(/&/gi, ",");
				...
				opWidth = 300;
				opHeight = 300;
				opUrl    = 'proses_pinjam.php?reqMode=setujui&reqId='+sData;
				var left = (screen.width/2)-(opWidth/2);
				var top = (screen.height/2)-(opHeight/2);
				newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
				newWindow.focus();			
        ...
        */
        function fnOnDeleting(tr, id, fnDeleteRow) {
			//alert(id);
            return confirm("Apakah anda yakin ingin menghapus data terpilih?"); 
        }
		
		function fnOnUpdatingRefRow(tr, id, fnUpdateRefRow) {//tak huapus
			//alert(id);
            return confirm("Setujui permohonan peminjaman?"); 
        }
        ...
        */
        ...
        return this.each(function () {
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;Tampil Berdasarkan <select id='bulan'><option value='all'>Semua Data</option><option value='peminjaman'>Data Peminjaman</option><option value='permohonan'>Data Permohonan</option><option value='disetujui'>Data Permohonan Disetujui</option><option value='ditolak'>Data Permohonan Ditolak</option></select>");	
				oBulanRowSelect = $("#bulan");
				
				oBulanRowSelect.change(function () { 
					//alert($("#bulan").val());	
					/* Example call to load a new file */
					fnReloadAjax('../json/pinjam_download-json.php?reqMode=' + $("#bulan").val());		

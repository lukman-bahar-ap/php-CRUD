...
(function ($) {
    ...
    $.fn.makeEditable = function (options) {
        ...
        var oAddNewRowButton, oDeleteRowButton, oBacaKeluarRowButton, oUpdateRefRowButton, oKembalikanRowButton,  oBacaRowButton, oBacaNotaDinasRowButton, 
		oDisposisiRowButton, oDisposisiNotaDinasRowButton, oTandaTerimaDisposisiRowButton, oEditEntryMasukRowButton, 
		oEditEntryKeluarRowButton, oEditNotaDinasRowButton, oSuratKeluarRowButton, oConfirmRowAddingButton, oCancelRowAddingButton, oEditArsipRowButton, oPublishArsipRowButton;
...
//			alert(mySplitResult[6]+'--');
		...
		function _fnOnRowPublishArsip(event) {
			...
			opWidth = 460;
			opHeight = 240;
			opUrl    = 'publish_add_monitoring.php?reqId='+id;
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
			newWindow.focus();			
		...
		function _fnOnRowFinaled(response) {
            properties.fnEndProcessingMode();
            var oTRSelected = $('tr.' + properties.sSelectedRowClass, oTable)[0];
			window.top.frames['mainFrameDetil'].location.href = 'json_entry_surat_keluar.php';
			//alert('asd');
            if (response == "ok" || response == "") {
                oTable.fnDeleteRow(oTRSelected);
        ...
        */
		...
		function fnOnFinalingRow(tr, id, fnFinalRow) {
			//alert(id);
            return confirm("Yakin Finalisasi Surat?"); ;
        }

...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready( function () {
		...
		if(ScreenX<=767)
		{
			varColumnDefs = [{ className: 'never', targets: [0] }, { className: 'none', targets: [0, -1, -2] }]
			varTinggi = 174;
		}
		else
		{
		...
		"sScrollX": "100%",
		"sScrollXInner": "100%",					  
		"sAjaxSource": "../json-setting/user_login_json.php?reqSatkerId=<?=$userLogin->userSatkerId?>&reqSatkerSubId=<?=$userLogin->userSatkerSubId?>",
		...
		$('#example tbody').on( 'click', 'tr', function () {
		   ...
		   // anSelectedSatkerId = element[element.length-2];
			...
			anSelectedGroup = element[1];
			anSelectedName = element[3];
		});	  			  
		...
		$('#btnUbah').on('click', function () {
			  ...
			  opUrl = 'user_login_add.php?reqMode=update&reqId='+anSelectedId+'&reqDevice='+anDevice;		
			  window.parent.OpenDHTML(opUrl, 'IT Online - Monitoring Ticket', widthPopup, heightPopup);	
		});
		$('#btnDevice').on('click', function () {
			  ...
			  opUrl = 'device_lookup.php?reqUserLoginId='+anSelectedId+'&reqDevice='+anDevice;		
			  window.parent.OpenDHTML(opUrl, 'IT Online - Monitoring Ticket', widthPopup, heightPopup);	
		});		  
		...
		$('#btnHapus').on('click', function () {
			...
			else if(anSelectedGroup == "Teknisi" || anSelectedGroup == "APO")
			{
				alert("Teknisi dan APO hanya bisa dihapus dari menu master data");
			}
			else
			{
		...
		$('#btnResetPassword').on('click', function () {
			...
			{
				$.messager.confirm('Warning', 'Apakah Anda Yakin Ingin Mereset Password dari '+anSelectedName+' ?', function(r){
					if (r){
						$.getJSON("../json-setting/user_reset_password.php?reqMode=user_login&reqId="+anSelectedId,
				  function(data){
					  location.reload(true);
						window.top.displayUrlFromIframeChild('../setting/user_login.php');
				  });
		...
		$('#btnResetDefaultPassword').on('click', function () {
			if(anSelectedData == ""){ return false; }
			else
			{
				$.messager.confirm('Warning', 'Apakah Anda Yakin Ingin Mereset Password dari '+anSelectedName+' ?', function(r){
					if (r){
						$.getJSON("../json-setting/user_reset_password.php?reqMode=reset_default&reqId="+anSelectedId,
				  function(data){
					  location.reload(true);
						window.top.displayUrlFromIframeChild('../setting/user_login.php');
				  });
...
function Tambah(){
	...
	window.parent.OpenDHTML('user_login_add.php?reqMode=insert&reqDevice='+anDevice, 'IT Online - Monitoring Ticket', widthPopup, heightPopup);
}
</script> 
...
<div id="wadah">
  ...
  <div class="panel-heading">
    <i class="fa fa-external-link-square"></i>
        <a href="#!" id="btnTambah" onClick="Tambah();" title="Tambah Menggunakan Personal Absen"><img src="../main/images/btn-tambah.png" width="15" height="15"/> Tambah </a>
        &nbsp;&nbsp;&nbsp;
		<a href="#!" id="btnUbah" title="Ubah"><img src="../main/images/btn-ubah.png" width="15" height="15"/> Ubah</a>
		&nbsp;&nbsp;&nbsp;
        <a href="#!" id="btnHapus" title="Hapus"><img src="../main/images/btn-hapus.png" width="15" height="15"/> Hapus</a>
        &nbsp;&nbsp;&nbsp;
        <a href="#!" data-flexmenu="flexmenu1" title="Reset Password"><img src="../main/images/btn-reload.png" width="15" height="15"/> Reset Password</a>
		&nbsp;&nbsp;&nbsp;
		<a href="#!" id="btnDevice" title="Device"><img src="../main/images/btn-hp.png" width="15" height="15"/> Device</a>
   </div>
   <ul id="flexmenu1" class="flexdropdownmenu">
        <li><a href="#!" id="btnResetPassword">&nbsp;&nbsp;&nbsp;&nbsp; Generate Password</a></li>
        <li><a href="#!" id="btnResetDefaultPassword">&nbsp;&nbsp;&nbsp;&nbsp; Default 123456</a></li>
    </ul>    
